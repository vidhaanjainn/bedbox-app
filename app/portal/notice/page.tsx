...
export default function NoticePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: res } = await supabase.from('residents').select('id, name').eq('portal_user_id', session.user.id).single()
      if (!res) { setLoading(false); return }
      setResident(res)
      const { data: notice } = await supabase.from('notice_periods').select('*').eq('resident_id', res.id).eq('status', 'active').maybeSingle()
  ...
  const handleSubmit = async () => {
    if (!confirmed || !vacateDate || !resident) return
    setSubmitting(true)
    const today = new Date().toISOString().split('T')[0]
    const { error: noticeError } = await supabase.from('notice_periods').insert({
      resident_id: resident.id,
      notice_date: today,
      last_day_of_stay: vacateDate,
      reason: reason || null,
      status: 'active',
      submitted_via: 'app',
    })
    ...
    return (
      <div style={{ padding: '28px 20px' }}>
        ...
        <div style={{ background: 'rgba(255,179,50,0.08)', border: '1px solid rgba(255,179,50,0.2)', borderRadius: 16, padding: 24, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.7 }}>Move-out date<br /><strong style={{ color: '#fff' }}>{endDate}</strong></div>
        </div>
  ...
  return (
    <div style={{ padding: '28px 20px' }}>
      ...
      <label style={{ display: 'flex', gap: 12, alignItems: 'flex-start', cursor: 'pointer', marginBottom: 24 }}>
        ...
        <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>I understand this starts my formal notice period immediately.</span>
      </label>
