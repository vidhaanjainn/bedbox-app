...
// Resident-gated: issues a signed upload URL for a meter-reading photo,
// mirroring payment-upload-url's pattern exactly.
export async function POST(req: Request) {
  ...
  const path = `electricity/${resident.id}/${year}-${month}-${Date.now()}`
